...
## 🔜 БЭКЛОГ (актуализирован 02.03.2026 — post Part 3)

### Приоритет 🔴 (ближайшие) — Фикс AI портфеля

1. **AI не заполняет бюджет** — Сценарий 3 использует ~51% бюджета. AI_PORTFOLIO_SELECTOR_PROMPT должен требовать ≥90%. Python добор лотов если <80%
2. **Одинаковые лоты в сценарии 3** — Дубли К1, одна цена. Диверсификация в промпте + Python валидация
3. **Ценовая непоследовательность** — Карточка со скидкой, AI текст без. Единообразие в format_portfolio_summary_v3()
4. **Сценарий 1 — остаток 50%** — Если лот покрывает <70% бюджета, reasoning должен объяснить

### Приоритет 🟡 (средний)

5. Портфельный PDF с 3 сценариями — проверить strategy_pdf_generator.py
6. Лотовый PDF с метриками — проверить
7. Версия в app.py — ✅ DONE (0.9.2 → 0.9.5)
8. История чата — сохранение сессий
9. Формат B метрик (IRR, NPV, Sensitivity Analysis)

### Приоритет 🟢 (nice-to-have)

10. К4 whitelist (инфраструктура готова)
11. Миграция на российский LLM
12. Автосинхронизация данных бот↔webapp
13. Админ-панель
---
