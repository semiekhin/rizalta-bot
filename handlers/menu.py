...
async def handle_start(chat_id: int, text: str = "", user_info: Dict[str, Any] = None):
    ...
    welcome = (
        "👋 Здравствуйте!\n\n"
        "Я онлайн-консультант проекта <b>RIZALTA Resort Belokurikha</b>.\n\n"
        "Могу помочь:\n"
        "📊 Рассчитать доходность по апартаментам;\n"
        "💳 Рассказать про рассрочку и ипотеку;\n"
        "📎 Показать планировки;\n"
        "ℹ️ Объяснить, почему Алтай — выгодная инвестиция.\n\n"
...
async def handle_why_altai(chat_id: int):
    ...
    text = (
        ...
        "🫰 Доходность, подтверждённая авторитетным источником CORE.XP, — как правило выше, чем у обычного жилья или апартаментов.\n"
...
async def handle_choose_unit_for_layout(chat_id: int):
    ...
    await send_message(
        chat_id,
        "📎 Выберите апартамент, чтобы получить планировку:",
        with_keyboard=True,
        buttons=UNIT_SELECT_BUTTONS,
    )
