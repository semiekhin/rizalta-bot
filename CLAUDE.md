...
### 🔴 Ближайшие
1. **Исправить AI промпты отчётов** — LOT_REPORT_PROMPT и PORTFOLIO_PROMPT дают нечитаемый ответ (сырой markdown с переменными JSON). Нужно: AI форматирует для человека, чистые цифры
2. **Адаптировать strategy_pdf_generator.py** — под данные report_builder. Страница 3 пустая в лотовом, портфельный PDF не скачивается
3. **Починить портфельный PDF** — strategy_data для портфеля не содержит нужных полей
...
8. К4 whitelist (инфраструктура готова)
