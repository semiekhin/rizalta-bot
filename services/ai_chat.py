...
AVAILABLE_FUNCTIONS = [
    ...
    },
    {
        ...
        "function": {
            "name": "calculate_roi",
            "description": "Рассчитать доходность конкретного апартамента. Вызывай когда пользователь спрашивает про доходность, ROI, сколько можно заработать на конкретном юните.",
            "parameters": {
                "type": "object",
                "properties": {
                    "unit_code": {
                        "type": "string",
                        "enum": ["A209", "B210", "A305"],
                        "description": "Код апартамента: A209 (студия), B210 (стандарт), A305 (люкс)"
                    }
                },
                "required": ["unit_code"]
            }
            ...
            "parameters": {
                ...
                "properties": {
                    "unit_code": {
                        "type": "string",
                        "enum": ["A209", "B210", "A305"],
                        "description": "Код апартамента (опционально)"
                    }
                },
                "required": []
            }
    ...
    },
    {
        ...
        "function": {
            "name": "book_showing",
            "description": "Записать на онлайн-показ апартаментов. Вызывай когда пользователь хочет записаться на показ, посмотреть апартаменты, связаться с менеджером, получить консультацию.",
            "parameters": {
                "type": "object",
                "properties": {},
                "required": []
    ...
    },
    {
        ...
        "function": {
            "name": "show_layouts",
            "description": "Показать планировки апартаментов. Вызывай когда пользователь просит планировку, схему, план квартиры.",
            "parameters": {
                "type": "object",
                "properties": {
                    "unit_code": {
    ...
    },
    {
        ...
        "function": {
            ...
            "description": "Получить коммерческое предложение (КП) на апартамент. Вызывай когда пользователь просит КП, коммерческое предложение, презентацию по лоту, предложение на конкретную площадь или бюджет.",
            "parameters": {
                "type": "object",
                "properties": {
                    "unit_code": {
                        "type": "string",
                        "description": "Код апартамента (например А209, В415)"
                    },
                    "area": {
                        "type": "number",
