# RIZALTA Bot — Описание проекта

## О проекте
AI-консультант для риэлторов. Инвестиционная недвижимость RIZALTA Resort Belokurikha (Алтай).

**Часть экосистемы RIZALTA AI PLATFORM:**
- ✅ **RIZALTA BOT** — цифровой отдел продаж 24/7 (готов)
- 🔜 **RealtMy Mini App** — управление контентом Telegram-каналов
- 🔜 **Контент-завод** — AI-медиацентр автогенерации

## Боты
- **Prod:** @RealtMeAI_bot (webhook, порт 8000)
- **Dev:** @rizaltatestdevop_bot (polling)

## Сервер
```bash
ssh -p 2222 root@72.56.64.91
```
- `/opt/bot` — prod
- `/opt/bot-dev` — dev

## Стек
Python 3.12 · FastAPI · OpenAI GPT-4o-mini · Whisper · SQLite · Cloudflare Tunnel

---

## Структура проекта
```
/opt/bot/
├── app.py                    # Главный файл (webhook, роутинг, startup_event)
├── run_polling.py            # Dev режим (polling, reminder_loop, monitoring_loop)
├── config/
│   └── settings.py           # Константы, кнопки меню, SHOWS_GROUP_ID
├── handlers/
│   ├── __init__.py           # Экспорты
│   ├── menu.py               # Главное меню
│   ├── ai_chat.py            # AI диалоги
│   ├── booking.py            # Онлайн-показы
│   ├── booking_fixation.py   # Фиксация клиентов ri.rclick.ru
│   ├── booking_calendar.py   # Календарь + групповые заявки (v2.1.2)
│   ├── compare.py            # Депозит vs RIZALTA
│   ├── kp.py                 # КП + универсальная навигация + пагинация
│   ├── calc_dynamic.py       # Расчёты ROI
│   ├── secretary.py          # AI-секретарь + timezone
│   ├── docs.py               # Документы
│   ├── news.py               # Новости/дайджест
│   ├── media.py              # Медиа
│   └── units.py              # Работа с лотами
├── services/
│   ├── telegram.py           # API Telegram + edit_message_inline (v2.1.2)
│   ├── notifications.py      # Уведомления + notify_shows_group (v2.1.2)
│   ├── ai_chat.py            # OpenAI интеграция
│   ├── calculations.py       # Финансовые расчёты
│   ├── monitoring.py         # Мониторинг нагрузки
│   ├── secretary_db.py       # БД секретаря + timezone
│   ├── intent_router.py      # GPT Intent Router
│   ├── rclick_service.py     # API ri.rclick.ru
│   ├── deposit_calculator.py # Калькулятор депозита ЦБ
│   ├── investment_compare.py # Сравнение инвестиций
│   ├── compare_pdf_generator.py # PDF сравнения
│   ├── kp_pdf_generator.py   # PDF КП
│   ├── calc_xlsx_generator.py # Excel ROI
│   ├── calc_universal.py     # Рассрочка 12/18 мес
│   ├── units_db.py           # Работа с БД лотов (348 лотов)
│   └── kp_resources/         # Логотип, шрифты
├── data/
│   ├── rizalta_knowledge_base.txt # База знаний AI
│   ├── units.json            # Данные лотов
│   └── rizalta_finance.json  # Финансовые данные
├── properties.db             # 348 лотов + bookings с taken_by
├── secretary.db              # Задачи + users (timezone)
└── monitoring.db             # Статистика запросов
```

---

## Функциональность

### 1. КП (Коммерческие предложения)
```
📋 Коммерческие предложения
├─ 🏢 По корпусу → Этаж → Лоты
├─ 📐 По площади
├─ 💰 По бюджету (±10% от суммы)
└─ 🔍 По номеру лота
```
- **3 варианта:** 100% оплата, 12 мес, 12+18 мес
- PDF с логотипом, планировкой, расчётами
- Скидка 5% при 100% оплате
- Пагинация "Показать ещё N лотов"

### 2. Расчёты доходности
- Динамический калькулятор ROI
- Excel генератор
- Рассрочка 12 и 18 месяцев

### 3. Сравнение депозит vs RIZALTA
- Данные ЦБ РФ (ключевая ставка)
- 3 сценария (базовый, оптимист, пессимист)
- PDF отчёт

### 4. AI-секретарь
- Создание задач голосом/текстом
- Напоминания за 15 минут (фоновая задача)
- 11 часовых поясов России (UTC+2 — UTC+12)
- Просмотр: сегодня, завтра, неделя

### 5. AI консультант
- GPT-4o-mini с Intent Router
- База знаний проекта
- Голосовой ввод (Whisper)
- Function Calling для действий

### 6. Фиксация клиентов
- Интеграция с ri.rclick.ru
- Авторизация риэлтора (токен 90 дней)

### 7. Групповые заявки на показ (v2.1.2)
- Заявка приходит в группу с кнопкой "🙋 Взять заявку"
- Кто первый нажал — забирает себе
- Сообщение обновляется "✅ Заявка взята — Имя"
- Автоматически создаётся задача в секретаре специалиста

### 8. Мониторинг
- Алерт >30 запросов/мин
- Алерт RAM >50%
- Ежедневный отчёт 20:00

---

## База данных

### properties.db — 348 лотов
- Корпус 1 «Family»: 244 лота
- Корпус 2 «Business»: 104 лота
- 70 дублей кодов (А200, А202 и др.)
- Таблица `bookings`: taken_by_id, taken_by_name, group_message_id (v2.1.2)

### secretary.db
- `tasks` — задачи пользователей
- `users` — timezone пользователей (default: 3 = Москва)

### monitoring.db
- `stats` — timestamp, user_id, request_type, response_time_ms

---

## Telegram Mini App (стратегия)

### Решение от 29.12.2025
Компонент **RealtMy** из RIZALTA AI PLATFORM будет реализован как **Telegram Mini App** вместо нативного приложения iOS/Android.

### Преимущества Mini App
| Критерий | Native App | Mini App |
|----------|-----------|----------|
| Стоимость | 800-1200 тыс ₽ | 300-400 тыс ₽ |
| Срок | 2-3 месяца | 3-4 недели |
| Установка | App Store/Play | Уже в Telegram |
| Кодовые базы | 2 (iOS + Android) | 1 (React) |

### Планируемые Mini Apps
1. **Шахматка** — визуальный выбор лотов (прототип готов)
2. **RealtMy** — генерация постов, публикация в каналы, CRM

### UI стиль Mini App (образец)
```jsx
// Цветовая схема
colors: {
  background: 'bg-slate-900',
  card: 'bg-slate-800', 
  accent: 'amber-500',
  text: 'text-white',
  muted: 'text-slate-400'
}

// Статусы лотов
status: {
  available: 'bg-emerald-500',  // Свободно
  booked: 'bg-amber-500',       // Бронь
  sold: 'bg-gray-600'           // Продано
}
```

### План реализации шахматки
1. Добавить `/api/lots` в app.py (30 мин)
2. React Mini App на Vercel (2-3 дня)
3. Регистрация в BotFather (5 мин)
4. Кнопка "🏠 Шахматка" в меню (10 мин)

---

## История изменений

### v2.1.2 (29.12.2025)
- Групповые заявки на показ с кнопкой "Взять"
- Интеграция заявок с AI-секретарём
- Функции edit_message_inline, send_message_inline_return_id
- Колонки taken_by_id, taken_by_name, group_message_id в bookings
- Прототип Mini App шахматки
- Стратегическое решение: RealtMy → Mini App

### v2.1.1 (24.12.2025)
- Мониторинг нагрузки с алертами
- Часовые пояса для секретаря (11 зон)
- Напоминания через фоновую задачу
- Убрана массовая генерация КП

### v2.1.0 (24.12.2025)
- 348 лотов вместо 69
- Универсальная навигация Корпус → Этаж → Лоты
- Пагинация "Показать ещё"
- Обработка 70 дублей кодов
- Поиск по бюджету ±10%
- Рассрочка 12/18 месяцев
- GPT Intent Router с приоритетами

### v1.9.6 (19.12.2025)
- Новый дизайн КП при 100% оплате
- 6 презентаций проекта
- 9 видео про Алтай

### v1.9.0 (ранее)
- AI-секретарь
- Фиксация клиентов rclick
- Сравнение с депозитом
